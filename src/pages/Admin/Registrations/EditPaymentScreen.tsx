   ...
   const { paymentId, registrationId } = route.params;
    ...
    return (
        <View style={styles.container}>
            ...
            <CustomPicker label="Forma de Pagamento" selectedValue={selectedPaymentMethodId} onValueChange={setSelectedPaymentMethodId} items={paymentMethods.map(pm => ({ label: pm.name, value: pm.id }))} />
            <CustomPicker label="Recebido por (Tesoureiro)" selectedValue={selectedTreasurerId} onValueChange={setSelectedTreasurerId} items={treasurers.map(t => ({ label: t.name, value: t.id }))} />
...
const styles = StyleSheet.create({
    ...
    input: { height: 50, backgroundColor: '#f5f5f5', borderColor: '#ccc', borderWidth: 1, borderRadius: 8, paddingHorizontal: 15, marginBottom: 20, fontSize: 16 },
